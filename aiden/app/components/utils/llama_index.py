...
from llama_index.core.query_engine import CitationQueryEngine, RouterQueryEngine
...
# @st.cache_resource(show_spinner=False)
def create_query_engines(_documents):
    """
    ...
    try:
        ...
        storage_context = StorageContext.from_defaults(persist_dir=os.getcwd() + "/embeddings")
        index = load_index_from_storage(storage_context)
    except Exception as e:
        logs.log.error(f"Error when creating index from documents: {e}")
        raise Exception(f"Error when creating index from documents: {e}")
    ...
    try:
        citation_query_engine = CitationQueryEngine.from_args(
            index, citation_chunk_size=512, streaming=True, memory=ChatMemoryBuffer.from_defaults()
        )

        st.session_state["citation_query_engine"] = citation_query_engine

        logs.log.info("Citation query Engine created successfully")
    except Exception as e:
        logs.log.error(f"Error when creating Citation Query Engine: {e}")
        raise Exception(f"Error when creating Citation Query Engine: {e}")

    return query_engine, citation_query_engine