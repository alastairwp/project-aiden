...
def format_sources(source_nodes: list[NodeWithScore]) -> str:
    ...
    for index, source_node in enumerate(source_nodes):
        if source_node.node.metadata['file_path'] not in sources_dict:
            sources_dict[source_node.node.metadata['file_path']] = []

        sources_dict[source_node.node.metadata['file_path']].append(index + 1)

    grouped_sources = sorted([(sources_dict[file_path], file_path) for file_path in sources_dict], key=lambda x: x[0])

    sources = [f"{source_indexes} {source}" for (source_indexes, source) in grouped_sources]
...
def context_chat(prompt: str, query_engine: CitationQueryEngine):
    """
    ...
    try:
        ...
        sources = format_sources(stream.source_nodes)

        for text in stream.response_gen:
            # print(str(text), end="", flush=True)
            # sources.add(str(stream.source_nodes[0].node.metadata["file_path"]))
            yield str(text)
        # yield [node.get_score() for node in stream.source_nodes]
...
def citation_chat(prompt: str, query_engine: CitationQueryEngine):
    """
    Initiates a chat with context using the Llama-Index query_engine.
